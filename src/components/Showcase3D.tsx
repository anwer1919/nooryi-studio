...
export default function Showcase3D() {
  ...
  return (
    ...
    >
      <div className="absolute top-10 right-1/4 w-72 h-72 bg-[#F5A623]/10 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-10 left-1/4 w-72 h-72 bg-[#F5A623]/10 rounded-full blur-3xl pointer-events-none"></div>
          ...
          <div className="relative h-[240px] sm:h-[320px] md:h-[400px] lg:h-[460px] mx-auto max-w-5xl" style={{ transformStyle: "preserve-3d" }}>
            {SLIDES.map((s, i) => {
              ...
              return (
                <div
                  key={i}
                  className="absolute inset-x-4 sm:inset-x-10 md:inset-x-16 top-0 bottom-0 rounded-3xl overflow-hidden border border-[#F5A623]/30 shadow-2xl shadow-[#F5A623]/10 transition-transform duration-[900ms]"
                  style={{
        ...
        <div className="flex justify-center gap-2 mt-8">
          {SLIDES.map((_, i) => (
            <button
              ...
              className={"h-2 rounded-full transition-all duration-300 " + (i === index ? "w-8 bg-[#F5A623]" : "w-2 bg-gray-300 dark:bg-white/20 hover:bg-[#F5A623]/50")}
            />
          ))}
        </div>
      </div>
