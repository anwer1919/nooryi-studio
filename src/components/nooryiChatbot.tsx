...
const QUICK_ACTIONS = [
  { label: "🎤 البحث عن فنان", value: "أريد البحث عن فنان مناسب" },
  { label: "📅 حجز فنان", value: "أريد حجز فنان" },
  { label: "💰 معرفة الأسعار", value: "ما هي أسعار الفنانين؟" },
  { label: "📋 حجوزاتي", value: "أريد متابعة حالة حجوزاتي" },
  { label: "❓ سؤال آخر", value: "" },
];
    ...
    async (t: string) => {
      ...
      try {
        const r = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            messages: [...msgs, u].map(({ role, content }) => ({ role, content })),
        ...
        while (true) {
          ...
          for (const ln of dc.decode(value, { stream: true }).split("\n")) {
            if (ln.startsWith("0:")) {
              try {
                const p = JSON.parse(ln.slice(2));
                if (typeof p === "string") {
                  tx += p;
                  setMsgs((prev) =>
                    prev.map((m) => (m.id === aid ? { ...m, content: tx } : m))
                  );
                }
              } catch {}
            }
        ...
        setMsgs((p) => [
          ...p,
          {
            ...
            role: "assistant",
            content: "عذراً، حصلت مشكلة مؤقتة. حاول مرة أخرى بعد قليل.",
          },
        ]);
      } finally {
        setLd(false);
      ...
      {open && (
        <div style={S.win}>
          ...
          {msgs.length === 0 && (
            <div style={S.qr}>
              {QUICK_ACTIONS.map((q, i) => (
                <button key={i} style={S.qb} onClick={() => q.value && send(q.value)}>
                  {q.label}
                </button>
              ))}
            </div>
          ...
          <div style={S.msgs} dir="rtl">
            {msgs.length === 0 && (
              <div style={{ textAlign: "center", color: "#999", fontSize: 14, marginTop: 40, lineHeight: 1.8 }}>
                مرحباً 👋
                <br />
                أنا مساعد Nooryi Studio.
                <br />
              ...
              <div key={m.id} style={{ display: "flex", gap: 8, flexDirection: m.role === "user" ? "row-reverse" : "row" }}>
                <div
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    background: m.role === "user" ? "#7c3aed" : "#F5A623",
                  }}
                >
          ...
          <form onSubmit={sub} style={{ padding: 12, borderTop: "1px solid rgba(245,166,35,0.15)", display: "flex", gap: 8 }} dir="rtl">
            ...
            <button type="submit" disabled={ld || !inp.trim()} style={{ ...S.send, opacity: ld || !inp.trim() ? 0.5 : 1 }}>
              <IconSend />
            </button>
          </form>
        </div>
