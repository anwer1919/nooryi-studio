    ...
    return (
      <div className="relative" ref={desktopMenuRef}>
        <button
          ...
          className="flex items-center gap-2 pl-2 pr-4 py-1.5 rounded-full bg-white/10 hover:bg-white/20 border border-line backdrop-blur-sm transition-all"
        >
          ...
          <span className="text-sm font-bold text-fg hidden sm:block max-w-[120px] truncate">
            {user.name}
          </span>
          <ChevronDown size={14} className={`text-muted transition-transform duration-200 ${isDesktopMenuOpen ? "rotate-180" : ""}`} />
        </button>

        {isDesktopMenuOpen && (
          <div className="absolute left-0 mt-2 w-64 bg-surface rounded-2xl shadow-2xl shadow-black/50 border border-line overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
            {/* User Info Header */}
            <div className="px-4 py-3 bg-gradient-to-r from-[#1a1a1a] to-[#111] border-b border-line">
              <p className="text-sm font-bold text-fg truncate">{user.name}</p>
              <p className="text-xs text-fg/50 truncate mt-0.5">{user.email}</p>
            ...
            <div className="p-1.5 space-y-0.5">
              <Link href="/my-bookings" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                ...
                <span className="text-sm font-semibold text-muted group-hover:text-fg">حسابي</span>
              </Link>
              <Link href="/my-bookings" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                <Calendar size={16} className="text-[#F5A623] group-hover:scale-110 transition-transform" />
                <span className="text-sm font-semibold text-muted group-hover:text-fg">حجوزاتي</span>
              </Link>
              <Link href="/artists" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                ...
                <span className="text-sm font-semibold text-muted group-hover:text-fg">تصفح الفنانين</span>
              </Link>
              ...
              {user.isAdmin && (
                <Link href="/admin" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                  ...
                  <span className="text-sm font-semibold text-muted group-hover:text-fg">لوحة التحكم</span>
                </Link>
              )}
              ...
              {user.role === "ARTIST_MANAGER" && (
                <Link href="/admin/artists-managers" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                  <LayoutDashboard size={16} className="text-[#F5A623] group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-semibold text-muted group-hover:text-fg">إدارة الفنانين</span>
                </Link>
              )}
              ...
              <Link href="/settings" onClick={() => setIsDesktopMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
                ...
                <span className="text-sm font-semibold text-muted group-hover:text-fg">الإعدادات</span>
              </Link>
  ...
  return (
    <>
      <button
        ...
        className="p-2 text-muted hover:text-fg hover:bg-white/10 rounded-xl transition-all"
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-bg shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        {/* Header */}
        <div className="bg-gradient-to-l from-surface to-bg p-6 border-b border-line">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-black text-fg">القائمة</h2>
            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 hover:bg-white/10 rounded-xl transition text-muted hover:text-fg" aria-label="إغلاق">
              <X size={20} />
            </button>
          </div>
        ...
        <div className="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-280px)]">
          <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
            ...
            <span className="font-semibold text-muted group-hover:text-fg">الرئيسية</span>
          </Link>
          <Link href="/artists" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
            ...
            <span className="font-semibold text-muted group-hover:text-fg">الفنانين</span>
          </Link>
          <Link href="/my-bookings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
            ...
            <span className="font-semibold text-muted group-hover:text-fg">حسابي</span>
          </Link>
          <Link href="/my-bookings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F5A623]/10 transition-colors group">
            <Calendar size={20} className="text-[#F5A623]" />
            <span className="font-semibold text-muted group-hover:text-fg">حجوزاتي</span>
          </Link>
        ...
        {/* Bottom Actions */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-line bg-bg space-y-2">
          <Link href="/settings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#F5A623]/10 transition-colors">
            ...
            <span className="font-semibold text-muted">الإعدادات</span>
          </Link>
