...
export default function AdminSidebarClient({ menuItems, userName, userRole }: { menuItems: MenuItem[]; userName: string; userRole: string }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 right-0 h-full w-72 bg-[#0a0a0a] border-l border-[#F5A623]/15 z-50 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-[#F5A623]/15">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div><h1 className="text-base font-black text-white">Nooryi</h1><p className="text-[8px] text-[#F5A623] font-bold tracking-[0.2em] uppercase">Admin</p></div>
              </div>
          ...
          <div className="p-3 border-b border-[#F5A623]/10">
            <div className="flex items-center gap-3 p-2.5 bg-[#111] rounded-xl border border-[#F5A623]/10">
              ...
              <div className="flex-1 min-w-0"><p className="font-bold text-white text-sm truncate">{userName}</p><p className="text-[10px] text-[#F5A623] font-bold">{userRole === "SUPER_ADMIN" ? "مدير عام" : userRole === "ADMIN" ? "إدارة" : "مدير أعمال"}</p></div>
            </div>
          </div>
          <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
            {menuItems.map((item) => {
              ...
              return (<Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg font-bold transition-all text-sm ${isActive ? "bg-gradient-to-l from-[#F5A623] to-[#E8961A] text-[#0a0a0a] shadow-lg shadow-[#F5A623]/20" : "text-gray-400 hover:bg-[#111] hover:text-[#F5A623]"}`}><Icon size={16} className={isActive ? "text-[#0a0a0a]" : "text-gray-500 group-hover:text-[#F5A623]"}/><span>{item.label}</span>{isActive && <div className="mr-auto w-1.5 h-1.5 rounded-full bg-[#0a0a0a]"/>}</Link>)
            })}
          </nav>
