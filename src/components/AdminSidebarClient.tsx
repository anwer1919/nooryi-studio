...
export default function AdminSidebarClient({ menuItems, userName, userRole }: { menuItems: MenuItem[]; userName: string; userRole: string }) {
  ...
  return (
    <>
      {isOpen && <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-40 lg:hidden" onClick={() => setIsOpen(false)} />}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)}
                  className={`group flex items-center gap-3 px-4 py-3 rounded-2xl font-bold transition-all ${isActive ? "bg-gradient-to-l from-[#D4AF37] to-[#b8941f] text-[#0a0a0a] shadow-lg shadow-[#D4AF37]/20" : "text-gray-400 hover:bg-[#111] hover:text-[#D4AF37]"}`}>
                  <Icon size={20} className={isActive ? "text-[#0a0a0a]" : "text-gray-500 group-hover:text-[#D4AF37]"} />
                  <span>{item.label}</span>
                  {isActive && <div className="mr-auto w-2 h-2 rounded-full bg-[#0a0a0a]"></div>}
                </Link>
              )
            })}
          </nav>
          <div className="p-4 border-t border-[#D4AF37]/10">
            <button onClick={() => signOut({ callbackUrl: "/" })} className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-red-400 hover:bg-red-500/10 font-bold transition">
