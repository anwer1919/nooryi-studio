...
export default async function MyBookingsPage({ searchParams }: { searchParams: Promise<{ new?: string; id?: string; success?: string }> }) {
  ...
  try {
    const userCustomers = await prisma.customer.findMany({
      where: { OR: [{ email: userEmail }, { userId }] },
      select: { id: true, phone: true }
    });
    const customerIds = userCustomers.map(c => c.id);
    const customerPhones = userCustomers.map(c => c.phone).filter(Boolean);
    const sc: any[] = [{ clientEmail: userEmail }, { userId }];
    if (customerIds.length > 0) sc.push({ customerId: { in: customerIds } });
    for (const phone of customerPhones) {
      const clean = phone.replace(/[^0-9]/g, "");
      if (clean.length >= 10) sc.push({ clientPhone: { contains: clean.slice(-10) } });
    }
    console.log("Search:", JSON.stringify(sc));
          ...
          {isNewBooking && (
            <div className="mb-6 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-6">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center flex-shrink-0"><span className="text-white text-2xl">✓</span></div>
                <div className="flex-1">
                  <h3 className="text-lg font-black text-green-900 mb-1">تم إرسال حجزك بنجاح! 🎉</h3>
                  <p className="text-sm text-green-800">حجزك الآن <strong>قيد المراجعة</strong>.</p>
                  {newBookingId && <p className="text-xs text-green-700 font-mono bg-green-100 inline-block px-3 py-1 rounded mt-2">رقم الحجز: {newBookingId.slice(0, 12)}...</p>}
                </div>
              </div>
            </div>
          )}
              ...
              return (
                <div key={b.id} className="card-pro overflow-hidden">
                  <div className="relative h-48 bg-gradient-to-br from-[#111] to-[#232323] overflow-hidden">
                    {b.artist?.profileImage ? (<img src={b.artist.profileImage} alt={b.artist.name} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center"><Music size={48} className="text-[#d4af37]/50" /></div>)}
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent"></div>
                    <div className="absolute top-4 right-4"><span className={`status-chip ${status.class}`}>{status.icon} {status.label}</span></div>
                  ...
                  <div className="p-5 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex items-start gap-2"><Calendar size={16} className="text-[#b8941f] mt-0.5" /><div><p className="text-xs text-gray-500">التاريخ</p><p className="text-sm font-bold text-gray-900">{b.date ? new Date(b.date).toLocaleDateString("ar-EG") : "—"}</p></div></div>
                      <div className="flex items-start gap-2"><Clock size={16} className="text-[#b8941f] mt-0.5" /><div><p className="text-xs text-gray-500">الوقت</p><p className="text-sm font-bold text-gray-900">{b.timeSlot || "—"}</p></div></div>
                    </div>
                    <div className="flex items-start gap-2"><MapPin size={16} className="text-[#b8941f] mt-0.5" /><div><p className="text-xs text-gray-500">المكان</p><p className="text-sm font-bold text-gray-900">{b.venue?.name || "—"}{b.venue?.city && <span className="text-gray-500"> • {b.venue.city}</span>}</p></div></div>
                    <div className="flex items-start gap-2"><DollarSign size={16} className="text-[#b8941f] mt-0.5" /><div className="flex-1"><p className="text-xs text-gray-500">المبلغ الإجمالي</p><p className="text-lg font-black text-gray-900">{(b.grossAmount || 0).toLocaleString()} ج.م</p></div></div>
