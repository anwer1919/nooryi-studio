...
import { Calendar, Clock, MapPin, DollarSign, Music, FileText, Printer } from "lucide-react";
...
export default async function MyBookingsPage({ searchParams }: { searchParams: Promise<{ new?: string; id?: string; success?: string }> }) {
  ...
  const gs = (s: string) => {
    ...
    if (["CONFIRMED", "APPROVED", "ACCEPTED"].includes(u)) return { l: "مؤكد", c: "status-confirmed", i: "✓" };
    if (["PENDING_APPROVAL", "PENDING"].includes(u)) return { l: "قيد المراجعة", c: "status-pending", i: "⏳" };
    if (["COMPLETED", "DONE"].includes(u)) return { l: "مكتمل", c: "status-completed", i: "✓" };
    return { l: "مرفوض", c: "status-rejected", i: "✕" };
  };
