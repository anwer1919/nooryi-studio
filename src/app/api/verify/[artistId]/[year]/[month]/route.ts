...
export async function GET(
  ...
  try {
    ...
    let allSlots: any[] = []
    try {
      allSlots = await prisma.availability.findMany({
        where: {
          artistId: artist.id,
          date: {
            gte: startOfMonth,
      ...
      // لو الجدول مفيهوش حقل date، نرجع تقويم فاضي
      allSlots = []
    }

    // بناء خريطة الأيام المحجوزة
    const bookedDates = new Set<string>()
    allSlots.forEach((slot: any) => {
      if (slot.isBooked && slot.date) {
        bookedDates.add(formatDateKey(new Date(slot.date)))
      }
    })
