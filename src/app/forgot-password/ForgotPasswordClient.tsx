...
export default function ForgotPasswordClient() {
  ...
  if (step === "done") return (
    <div className="py-8 text-center">
      ...
      <h2 className="text-xl font-black text-fg mb-2">تم تغيير كلمة المرور!</h2>
      <p className="text-muted text-sm">جاري تحويلك لتسجيل الدخول...</p>
    </div>
  )
    ...
    return (
      <form onSubmit={handleReset} className="space-y-4">
        <div className="text-center mb-2">
          ...
          <h2 className="text-xl font-black text-fg mb-1">إعادة تعيين كلمة المرور</h2>
          <p className="text-muted text-xs">أدخل الرمز المرسل إلى <span className="text-fg font-bold">{destination}</span> وكلمة المرور الجديدة</p>
        </div>
        ...
        <div className="flex justify-center gap-2" dir="ltr">
          {otpDigits.map((d, i) => (<input key={i} ref={el => { otpRefs.current[i] = el }} type="text" inputMode="numeric" maxLength={1} value={d} onChange={e => handleOtpChange(i, e.target.value)} onKeyDown={e => handleOtpKeyDown(i, e)} className="text-center text-xl font-black text-fg bg-card border-2 border-[#F5A623]/20 rounded-xl focus:border-[#F5A623] focus:ring-2 focus:ring-[#F5A623]/30 outline-none transition-all" style={{ width: "2.6rem", height: "3rem" }}/>))}
        </div>
        <div className="relative">
          <Lock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted"/>
          <input type={showPass ? "text" : "password"} required minLength={6} value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full pr-9 pl-9 py-3 border border-[#F5A623]/20 bg-card rounded-xl focus:ring-2 focus:ring-[#F5A623]/30 focus:border-[#F5A623] outline-none transition text-fg text-sm placeholder:text-muted" placeholder="كلمة المرور الجديدة"/>
          <button type="button" onClick={() => setShowPass(!showPass)} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted hover:text-muted">{showPass ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
        </div>
        <div className="relative">
          <Lock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted"/>
          <input type={showPass ? "text" : "password"} required minLength={6} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full pr-9 pl-4 py-3 border border-[#F5A623]/20 bg-card rounded-xl focus:ring-2 focus:ring-[#F5A623]/30 focus:border-[#F5A623] outline-none transition text-fg text-sm placeholder:text-muted" placeholder="تأكيد كلمة المرور"/>
        </div>
        ...
        <div className="text-center">
          <button type="button" onClick={sendOtp} disabled={resendTimer > 0 || loading} className="text-xs font-bold text-[#E8961A] hover:text-[#F5A623] transition disabled:text-muted">
            {resendTimer > 0 ? `إعادة الإرسال بعد ${resendTimer}ث` : "إعادة إرسال الرمز"}
          </button>
          <button type="button" onClick={() => { setStep("email"); setOtpDigits(["", "", "", "", "", ""]); setError("") }} className="block mx-auto mt-2 text-xs text-muted hover:text-muted transition">تغيير البريد</button>
        </div>
      </form>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <div className="w-14 h-14 rounded-2xl bg-[#F5A623]/10 flex items-center justify-center mx-auto mb-3"><ShieldCheck size={28} className="text-[#F5A623]"/></div>
        <h2 className="text-xl font-black text-fg mb-1">نسيت كلمة المرور؟</h2>
        <p className="text-muted text-xs">أدخل بريدك الإلكتروني وسنرسل لك رمز التحقق لإعادة التعيين</p>
      </div>
      {error && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-sm text-red-400 text-center">{error}</div>}
      <div className="relative">
        <Mail size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted"/>
        <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full pr-9 pl-4 py-3 border border-[#F5A623]/20 bg-card rounded-xl focus:ring-2 focus:ring-[#F5A623]/30 focus:border-[#F5A623] outline-none transition text-fg text-sm placeholder:text-muted" placeholder="example@email.com"/>
      </div>
