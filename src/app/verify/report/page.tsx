      ...
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-[#F5A623]">
          <div className="bg-gradient-to-l from-[#F5A623] to-[#E8961A] p-6 text-center">
            <div className="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur flex items-center justify-center mx-auto mb-3"><ShieldCheck size={28} className="text-fg" /></div>
            <h1 className="text-2xl font-black text-white">صفحة التحقق من صحة التقرير</h1>
            <p className="text-muted text-sm mt-1">Nooryi Studio — نظام التحقق الرسمي</p>
          </div>
          {!artist || !stats ? (
            <div className="p-10 text-center"><p className="text-red-600 font-bold text-lg mb-2">⚠️ التقرير غير موجود</p><p className="text-muted text-sm">لم يتم العثور على فنان مطابق لرابط التحقق.</p></div>
          ) : (
            <div className="p-6 md:p-8">
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200 mb-6">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#F5A623] to-[#E8961A] flex items-center justify-center text-fg text-xl font-black flex-shrink-0">{(artist.name || "ف").charAt(0)}</div>
                <div className="flex-1 min-w-0"><p className="font-black text-lg text-gray-900 truncate flex items-center gap-2"><Music size={16} className="text-[#E8961A]" /> {artist.name}</p><p className="text-xs text-muted">{artist.category || "فنان"} • <span className="inline-flex items-center gap-1"><Star size={11} className="text-[#F5A623] fill-[#F5A623]" /> {Number(stats.rating).toFixed(1)} ({stats.ratingCount} تقييم)</span></p></div>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {[{ l: "إجمالي الحجوزات", v: String(stats.total) }, { l: "حجوزات مؤكدة", v: String(stats.confirmed) }, { l: "حجوزات مكتملة", v: String(stats.completed) }, { l: "إجمالي الإيرادات", v: stats.gross.toLocaleString() + " ج.م" }, { l: "عمولة المنصة", v: stats.commission.toLocaleString() + " ج.م" }, { l: "صافي الفنان", v: stats.net.toLocaleString() + " ج.م" }].map((x, i) => (
                  <div key={i} className="p-3 bg-gray-50 rounded-xl border border-gray-200 text-center"><p className="text-[10px] text-muted font-bold mb-1">{x.l}</p><p className="font-black text-gray-900">{x.v}</p></div>
                ))}
              </div>
              <p className="text-[11px] text-muted text-center leading-relaxed">هذه الأرقام محسوبة لحظياً من قاعدة بيانات المنصة وهي المرجع الرسمي لصحة أي تقرير مالي صادر لأي فنان.</p>
            </div>
          )}
          <div className="bg-gray-50 px-6 py-4 text-center border-t border-gray-200"><p className="text-[10px] text-muted">© {new Date().getFullYear()} Nooryi Studio — جميع الحقوق محفوظة | سجل تجاري 123456789</p></div>
        </div>
      </div>
    </div>
  )
