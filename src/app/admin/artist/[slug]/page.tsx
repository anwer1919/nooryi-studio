...
export default async function ArtistDashboard({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const stats = [
    ...
    { label: "متوسط التقييم", value: (reviewsAvg._avg.rating || 0).toFixed(1), icon: Star, color: "from-[#F5A623] to-[#E8961A]" },
  ...
  return (
    <div dir="rtl" className="space-y-6">
      <div className="flex items-center gap-4">
        {artist.profileImage ? (
          <img src={artist.profileImage} alt="" className="w-16 h-16 rounded-2xl object-cover border-2 border-[#F5A623]" />
        ) : (
