    ...
    <div dir="rtl" className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2"><Award size={24} className="text-[#F5A623]" /> التقرير المالي — {artist.name}</h2>
        ...
        <div className="p-5 hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center"><p className="text-xs text-gray-500">عمولة المنصة</p><p className="font-black text-[#F5A623]">{Math.round(data.revenue*data.commission/100).toLocaleString()} ج.م</p></div>
        ...
        <div className="p-5 border-t border-gray-200 dark:border-gray-800">
          ...
          <div className="flex items-center gap-4"><span className="text-5xl font-black text-[#F5A623]">{Number(data.rating).toFixed(1)}</span><div><div className="flex gap-1">{[1,2,3,4,5].map(i=><Star key={i} size={20} className={i<=Math.round(data.rating)?"text-[#F5A623] fill-[#F5A623]":"text-gray-300"}/>)}</div><p className="text-sm text-gray-500 mt-1">{data.ratingCount} تقييم</p></div></div>
        </div>
