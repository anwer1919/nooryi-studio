"use client"
import { DollarSign, TrendingUp, CheckCircle2, Clock, Star, Printer, Download, Share2, Filter, Calendar, MapPin, User, Phone } from "lucide-react"
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-100 p-4 md:p-8 font-sans">
      ...
      <div className="no-print max-w-[210mm] mx-auto mb-6 flex gap-3 justify-end flex-wrap">
        ...
        <button onClick={() => window.print()} className="flex items-center gap-2 px-6 py-3 bg-black text-white rounded-xl font-bold shadow-xl hover:bg-gray-800 transition"><Download size={20}/> حفظ / طباعة</button>
      </div>
      ...
      <div className="print-container max-w-[210mm] mx-auto bg-white shadow-2xl p-12 md:p-16 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl border-2 border-black">
            ...
            <div className="flex items-center gap-4">
              {artist.profileImage ? (
                ...
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#D4AF37] to-[#b8941f] flex items-center justify-center"><span className="text-[#111] text-2xl font-black">{artist.name.charAt(0)}</span></div>
              )}
              <div>
                <p className="text-xl font-bold text-black">{artist.name}</p>
                <p className="text-sm text-gray-600">{artist.category || "فنان"}</p>
                <p className="text-xs text-gray-500 mt-1">عمولة المنصة: {data.commissionRate}%</p>
              </div>
            </div>
          </div>
          <div className="md:text-left">
            <div className="inline-grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
              ...
              <span className="font-bold text-black">{data.total}</span>
              <span className="text-gray-500">مؤكدة:</span>
              <span className="font-bold text-black">{data.confirmed}</span>
              <span className="text-gray-500">مكتملة:</span>
              <span className="font-bold text-black">{data.completed}</span>
              <span className="text-gray-500">التقييم:</span>
              <span className="font-bold text-black">{Number(data.rating).toFixed(1)} ⭐ ({data.ratingCount})</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-black p-5 rounded-xl text-center">
              ...
              <p className="text-xl font-black text-[#D4AF37]">{data.revenue.toLocaleString()}</p>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border-2 border-black text-center">
              ...
              <p className="text-xs text-gray-500 mb-1">حجوزات مؤكدة</p>
              <p className="text-xl font-black text-black">{data.confirmed}</p>
            </div>
            <div className="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border-2 border-black text-center">
              <Clock className="w-8 h-8 text-yellow-600 mx-auto mb-2"/>
              <p className="text-xs text-gray-500 mb-1">قيد المراجعة</p>
              <p className="text-xl font-black text-black">{data.pending}</p>
            </div>
            <div className="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border-2 border-black text-center">
              <TrendingUp className="w-8 h-8 text-[#D4AF37] mx-auto mb-2"/>
              <p className="text-xs text-gray-500 mb-1">صافي الفنان</p>
              <p className="text-xl font-black text-black">{data.net.toLocaleString()}</p>
        ...
        <div className="mb-12">
          ...
          {data.recent.length === 0 ? (
            <p className="text-gray-400 text-center py-6">لا توجد حجوزات مؤكدة</p>
          ) : (
            <table className="w-full border-collapse text-sm">
              <thead><tr className="bg-[#0a0a0a] text-white">
                <th className="px-3 py-2.5 text-right text-xs font-bold">#</th>
                <th className="px-3 py-2.5 text-right text-xs font-bold">العميل</th>
                <th className="px-3 py-2.5 text-center text-xs font-bold">التاريخ</th>
                <th className="px-3 py-2.5 text-right text-xs font-bold">المكان</th>
                <th className="px-3 py-2.5 text-center text-xs font-bold">المبلغ</th>
              </tr></thead>
              <tbody>{data.recent.map((b: any, i: number) => (
                <tr key={b.id} className={`border-b border-gray-200 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  <td className="px-3 py-2.5 text-xs text-gray-500 font-mono">{String(i+1).padStart(2,'0')}</td>
                  <td className="px-3 py-2.5 font-bold">{b.clientName}</td>
                  <td className="px-3 py-2.5 text-center">{b.date ? new Date(b.date).toLocaleDateString("ar-EG") : "—"}</td>
                  <td className="px-3 py-2.5">{b.venue?.name || "—"}</td>
                  <td className="px-3 py-2.5 text-center font-black text-[#D4AF37]">{Number(b.grossAmount||0).toLocaleString()} ج.م</td>
                </tr>
              ))}</tbody>
            </table>
          )}
        ...
        <div className="flex justify-end mb-16">
          <div className="w-full md:w-[400px] rounded-2xl border-2 border-black overflow-hidden shadow-xl">
            <div className="flex justify-between py-4 px-6 border-b border-gray-200">
              ...
              <span className="font-bold text-black text-lg">{data.revenue.toLocaleString()} ج.م</span>
            </div>
            <div className="flex justify-between py-4 px-6 border-b border-gray-200 bg-gray-50">
              <span className="text-gray-600 font-bold">عمولة المنصة ({data.commissionRate}%):</span>
              <span className="font-bold text-red-600 text-lg">-{data.commission.toLocaleString()} ج.م</span>
            </div>
            <div className="flex justify-between py-6 px-6 bg-black text-white">
              <span className="font-bold text-xl text-[#D4AF37]">صافي الفنان:</span>
              <span className="font-black text-3xl text-[#D4AF37]">{data.net.toLocaleString()} ج.م</span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <ul className="text-xs text-gray-600 space-y-2">
                ...
                <li className="flex gap-2"><span className="text-[#D4AF37] font-bold">•</span><span>الأسعار قابلة للتغيير دون إشعار مسبق.</span></li>
              </ul>
            </div>
          ...
          <div className="flex justify-center pt-6 border-t border-gray-200">
            <div className="text-center">
              <div className="bg-white p-3 rounded-xl border-2 border-black inline-block shadow-lg">
                <QRCode value={verifyUrl} size={80} level="H" bgColor="#FFFFFF" fgColor="#000000"/>
              </div>
