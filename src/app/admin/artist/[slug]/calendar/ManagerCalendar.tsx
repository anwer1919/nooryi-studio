  ...
  return (
    <div className="print-area space-y-4">
      ...
      <div className="flex items-center gap-4 flex-wrap no-print">
        ...
        <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-gradient-to-br from-[#D4AF37] to-[#b8941f]"></div><span className="text-xs text-gray-400">اليوم</span></div>
      ...
      <div className="bg-[#111] rounded-2xl border border-[#D4AF37]/20 overflow-hidden">
        <div className="grid grid-cols-7">
          {DAYS_AR.map(d => (
            <div key={d} className="bg-[#0a0a0a] text-[#D4AF37] text-center py-3 font-black text-sm border-b border-[#D4AF37]/20">{d}</div>
          ))}
          {days.map((day, i) => {
            ...
            return (
              <button key={key} onClick={() => setSelected(isSelected ? null : key)}
                className={`aspect-square flex flex-col items-center justify-center relative transition-all border border-[#D4AF37]/5
                ...
                `}>
                <span className={`text-lg font-black ${isBooked ? "text-red-400" : isToday ? "text-[#D4AF37]" : "text-gray-300"}`}>{day.getDate()}</span>
      ...
      {selected && (
        <div className="bg-[#111] rounded-2xl p-5 border border-[#D4AF37]/20 no-print">
