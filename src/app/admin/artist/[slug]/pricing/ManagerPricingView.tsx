    ...
    <>
      ...
      <div dir="rtl" className="p-6 space-y-6 max-w-6xl mx-auto">
        ...
        <div className="no-print card-pro p-6">
          <h2 className="text-xl font-black text-gray-900 dark:text-white mb-4 flex items-center gap-2"><MapPin size={20} className="text-[#F5A623]"/> مناطق التسعير</h2>
        ...
        {/* Add/Edit Form */}
        <div className="no-print card-pro p-6">
          <h3 className="text-lg font-black text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Plus size={18} className="text-[#F5A623]"/>{editingId ? "تعديل المنطقة" : "إضافة منطقة جديدة"}</h3>
          ...
          <div id="print-report" className={` ${showPrintPreview ? '' : 'block'}`}>
            ...
            <div className="px-12 pt-10 pb-6 bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] text-white">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-5"><div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#F5A623] to-[#E8961A] flex items-center justify-center shadow-2xl"><span className="text-[#111] text-4xl font-black">N</span></div><div><h1 className="text-4xl font-black">{STUDIO_INFO.nameAr}</h1><p className="text-[#F5A623] font-bold mt-1">{STUDIO_INFO.name}</p><p className="text-xs text-gray-400 mt-2">{STUDIO_INFO.tagline}</p></div></div>
                <div className="text-right"><div className="inline-block px-4 py-2 bg-[#F5A623]/20 border border-[#F5A623] rounded-lg"><p className="text-xs text-[#F5A623] font-bold">تقرير أسعار</p><p className="text-xs text-gray-300 mt-1 font-mono" dir="ltr">{reportId}</p></div></div>
              </div>
            ...
            <div className="px-12 py-8 bg-[#faf8f0] border-b-4 border-[#F5A623]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">{artist.profileImage ? <img src={artist.profileImage} alt="" className="w-16 h-16 rounded-2xl object-cover shadow-lg"/> : <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#F5A623] to-[#E8961A] flex items-center justify-center shadow-lg"><span className="text-[#111] text-3xl font-black">{artist.name.charAt(0)}</span></div>}<div><p className="text-xs text-gray-500 font-bold uppercase">أسعار</p><h2 className="text-3xl font-black text-gray-900">{artist.name}</h2><p className="text-sm text-gray-600 mt-1">{artist.category||"فنان"}</p></div></div>
            ...
            <div className="px-12 py-8"><div className="grid grid-cols-3 gap-4">
              <div className="border-2 border-[#F5A623] rounded-2xl p-5 bg-gradient-to-br from-[#F5A623]/5 to-transparent"><p className="text-xs text-gray-600 font-bold uppercase mb-2">عدد المناطق</p><p className="text-4xl font-black text-gray-900">{totalRegions}</p></div>
              <div className="border-2 border-gray-200 rounded-2xl p-5 bg-white"><p className="text-xs text-gray-600 font-bold uppercase mb-2">متوسط السعر</p><p className="text-3xl font-black text-[#F5A623]">{avgPrice.toLocaleString()}</p><p className="text-xs text-gray-500 mt-1">جنيه مصري</p></div>
            ...
            <div className="px-12 py-10 bg-gradient-to-b from-white to-[#faf8f0]">
              <div className="grid grid-cols-3 gap-8 items-center">
                ...
                <div className="flex flex-col items-center justify-center"><div className="relative"><div className="w-32 h-32 rounded-full border-4 border-[#F5A623] flex items-center justify-center" style={{transform:'rotate(-15deg)',boxShadow:'inset 0 0 0 2px #F5A623, 0 0 0 2px #F5A623'}}><div className="text-center"><p className="text-[8px] font-bold text-[#F5A623] uppercase tracking-widest">{STUDIO_INFO.name}</p><p className="text-xs font-black text-[#F5A623] my-1">✦ معتمد ✦</p><p className="text-[10px] font-black text-[#F5A623]">APPROVED</p></div></div><div className="absolute inset-0 rounded-full border-2 border-[#F5A623]" style={{transform:'rotate(-15deg) scale(1.15)',opacity:0.5}}></div></div><p className="text-[10px] text-gray-500 mt-3 font-bold uppercase">ختم المنصة الرسمي</p></div>
                <div className="flex flex-col items-center"><div className="bg-white p-3 rounded-xl border-2 border-[#F5A623] shadow-lg"><QRCode value={verifyUrl} size={100} level="H" bgColor="#FFFFFF" fgColor="#0a0a0a"/></div><p className="text-[10px] text-gray-500 mt-2 font-bold uppercase text-center">امسح للتحقق</p></div>
              </div>
              <div className="mt-8 h-0.5 bg-gradient-to-r from-transparent via-[#F5A623] to-transparent"></div>
