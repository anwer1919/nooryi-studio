  ...
  return (
    <div dir="rtl" className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3">
        {stats.map((s, i) => (<div key={i} className={`stat-card ${s.accent === "dark" ? "dark" : ""}`}><div className="flex items-center justify-between mb-2"><span className="stat-label text-[9px] md:text-[10px]">{s.label}</span><s.icon size={14} className={s.accent === "dark" ? "text-[#D4AF37]" : "text-[#b8941f]"}/></div><div className="stat-value text-lg md:text-xl lg:text-2xl">{s.value}</div></div>))}
      </div>
      <div className="grid lg:grid-cols-3 gap-3 md:gap-4">
        <div className="lg:col-span-1 card-pro p-4 md:p-5">
          <h2 className="text-base md:text-lg font-black text-white mb-3 md:mb-4 flex items-center gap-2"><TrendingUp size={16} className="text-[#b8941f]"/> روابط سريعة</h2>
          <div className="space-y-1">
            {quickLinks.map((q, i) => (<Link key={i} href={q.href} className="flex items-center gap-2.5 p-2 md:p-2.5 rounded-lg hover:bg-[#1a1a1a] transition group"><div className="icon-circle dark w-8 h-8"><q.icon size={14}/></div><div className="flex-1 min-w-0"><p className="font-bold text-white text-xs md:text-sm group-hover:text-[#b8941f] transition">{q.label}</p><p className="text-[9px] md:text-[10px] text-gray-500 truncate">{q.desc}</p></div><ArrowLeft size={12} className="text-gray-600 group-hover:text-[#b8941f] transition flex-shrink-0"/></Link>))}
          </div>
        </div>
        <div className="lg:col-span-2 card-pro p-4 md:p-5">
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <h2 className="text-base md:text-lg font-black text-white flex items-center gap-2"><Clock size={16} className="text-[#b8941f]"/> آخر الحجوزات</h2>
            <Link href="/admin/bookings" className="text-[10px] md:text-xs font-bold text-[#b8941f] hover:text-[#D4AF37] transition">عرض الكل</Link>
          </div>
