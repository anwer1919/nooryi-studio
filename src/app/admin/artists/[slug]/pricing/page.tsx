...
export default async function ArtistPricingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let a: any = null;
  try { a = await prisma.artist.findFirst({ where: { OR: [{ id: slug }, { slug }] }, include: { pricing: true } }); } catch {}
  if (!a) notFound();
  const prices = Array.isArray(a.pricing) ? a.pricing : a.pricing ? [a.pricing] : [];
  return (
    <PrintLayout title={`تسعيرة ${a.name||"الفنان"}`} docNumber={`PRC-${(a.id||"").slice(0,8).toUpperCase()}`} verificationCode={a.id}>
      <div className="dash-card"><h3 style={{color:"#D4AF37",marginBottom:10,fontSize:"12pt",fontWeight:800,borderBottom:"1px solid #D4AF37",paddingBottom:6}}>بيانات الفنان</h3>
        <table className="print-info-table"><tr><td>الاسم</td><td>{a.name||"—"}</td></tr><tr><td>التصنيف</td><td>{a.category||"—"}</td></tr>{a.phone&&<tr><td>الهاتف</td><td dir="ltr">{a.phone}</td></tr>}</table></div>
