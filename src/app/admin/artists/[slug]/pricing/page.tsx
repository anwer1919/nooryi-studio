...
export default async function ArtistsPricingPrintPage({ params }: { params: Promise<{ slug?: string }> }) {
    ...
    <PrintLayout title={`تسعيرة ${artist.name || "الفنان"}`} docNumber={`PRC-${(artist.id||"").slice(0,8).toUpperCase()}`} verificationCode={artist.id}>
      ...
      {prices.length > 0 ? (
        <table className="print-table" style={{ marginTop: 20 }}>
          <thead><tr><th>البند</th><th>السعر</th><th>المدة / الوحدة</th><th>ملاحظات</th></tr></thead>
          <tbody>
