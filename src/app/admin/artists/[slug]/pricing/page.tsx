    ...
    <PrintLayout title={`تسعيرة ${artist.name || "الفنان"}`} docNumber={`PRC-${(artist.id||"").slice(0,8).toUpperCase()}`} verificationCode={artist.id}>
      <div className="dash-card">
        ...
        <table className="print-info-table">
          ...
          {artist.phone && <tr><td>الهاتف</td><td dir="ltr">{artist.phone}</td></tr>}
        </table>
      </div>
      {prices.length > 0 ? (
        <table className="print-table" style={{ marginTop: 20 }}>
          <thead><tr><th>البند</th><th>السعر</th><th>المدة</th><th>ملاحظات</th></tr></thead>
          <tbody>{prices.map((p: any, i: number) => (<tr key={i}><td>{p.title || p.name || p.serviceName || "—"}</td><td style={{ fontWeight: 700 }}>{Number(p.price || p.amount || 0).toLocaleString()}</td><td>{p.duration || p.unit || p.period || "—"}</td><td>{p.notes || p.description || "—"}</td></tr>))}</tbody>
        </table>
