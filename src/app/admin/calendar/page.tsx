...
import {
  Save, Printer, Calendar,
  ChevronRight, ChevronLeft, Filter, Clock,
  Phone, Mail, MapPin, Award, Shield
} from "lucide-react"
...
export default function AdminCalendarPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const slotsRes = await fetch(
        `/api/admin/artists/${selected.slug}/availability?year=${year}&month=${monthIndex + 1}`
      )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/artists/${selected.slug}/availability`, {
        method: "POST",
          ...
          <div className="mb-6 no-print">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                ...
                <p className="text-gray-500">إدارة أيام العمل للفنانين</p>
              </div>

              {/* ✅ أزرار الطباعة والحفظ - واضحة وبارزة */}
              <div className="flex gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-4 bg-[#D4AF37] text-[#111] rounded-xl font-black text-lg hover:bg-[#b8941f] transition disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                >
                  <Save size={22} />
            ...
            <>
              ...
              <div className="mt-6 p-4 bg-[#111] text-[#D4AF37] rounded-xl no-print">
                <p className="text-sm font-semibold flex items-center gap-2">
                  <Clock size={16} />
                  اضغط على أي يوم لتحديده كمتاح أو غير متاح • الأيام الحمراء محجوزة ولا يمكن تعديلها • اضغط "طباعة التقويم" للحصول على نسخة احترافية بحجم A4
                </p>
              </div>
            </>
          )}
