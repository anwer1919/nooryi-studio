    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-md">
          ...
          <button onClick={() => router.back()} className="px-8 py-3 bg-black text-white rounded-xl font-bold">العودة</button>
        </div>
      </div>
    )
  }
    ...
    <div dir="rtl" className="min-h-screen bg-gray-100 p-4 md:p-8 font-sans">
      ...
      <div className="print-container max-w-[210mm] mx-auto bg-white shadow-2xl p-12 md:p-16 relative overflow-hidden">
                ...
                <div key={idx}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className={`${item.color} h-2 rounded-full`}
                      style={{ width: `${reportData.length > 0 ? (item.value / reportData.length) * 100 : 0}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t border-gray-200 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="font-semibold text-gray-700">إجمالي العربون:</span>
        ...
        {stats?.byArtist && Object.keys(stats.byArtist).length > 0 && (
          <div className="mb-12">
            <h3 className="text-lg font-black text-black mb-4 pb-2 border-b-2 border-[#D4AF37]">أداء الفنانين</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* الجدول التفصيلي */}
        <div className="mb-12">
          <h3 className="text-lg font-black text-black mb-4 pb-2 border-b-2 border-[#D4AF37]">التفاصيل الكاملة</h3>
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-black text-white">
                <th className="py-4 px-2 text-right font-bold w-10">#</th>
                <th className="py-4 px-2 text-right font-bold">الفنان</th>
                <th className="py-4 px-2 text-right font-bold">العميل</th>
                <th className="py-4 px-2 text-right font-bold">التاريخ</th>
                <th className="py-4 px-2 text-right font-bold">الحالة</th>
                <th className="py-4 px-2 text-right font-bold">الدفع</th>
                <th className="py-4 px-2 text-left font-bold text-[#D4AF37]">المبلغ</th>
              </tr>
            </thead>
            <tbody className="text-gray-700">
              {reportData.map((b: any, i: number) => {
                ...
                const pl = rem === 0 && dep > 0
                  ? { t: "مدفوع", c: "bg-green-100 text-green-700" }
                  : dep > 0
                  ? { t: "جزئي", c: "bg-blue-100 text-blue-700" }
                  : { t: "غير مدفوع", c: "bg-red-100 text-red-700" }

                return (
                  <tr key={b.id} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                    <td className="py-3 px-2 text-gray-500 font-mono font-bold">{i + 1}</td>
                    <td className="py-3 px-2 font-bold text-black text-xs">{b.artist?.name || "غير محدد"}</td>
                    ...
                    <td className="py-3 px-2"><span className={`px-2 py-1 rounded text-[10px] font-bold ${pl.c}`}>{pl.t}</span></td>
                    <td className="py-3 px-2 text-left font-bold text-black">{Number(b.grossAmount || 0).toLocaleString()}</td>
                  </tr>
                )
              })}
              {reportData.length === 0 && (
                <tr><td colSpan={7} className="py-12 text-center text-gray-500 italic">لا توجد حجوزات مطابقة</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <ul className="text-xs text-gray-600 space-y-2">
                ...
                <li className="flex gap-2"><span className="text-[#D4AF37] font-bold">•</span><span>جميع المبالغ بالجنيه المصري (EGP).</span></li>
                <li className="flex gap-2"><span className="text-[#D4AF37] font-bold">•</span><span>يمكن التحقق من صحته عبر مسح QR أدناه.</span></li>
              </ul>
            </div>
