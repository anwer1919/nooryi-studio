...
export default function AdminSidebar({ userRole, userName, userEmail }: any) {
  ...
  // ✅ الحل المطلق لـ Hydration
  if (!isMounted) {
    return (
      <div className="lg:hidden" style={{ position: "sticky", top: 0, zIndex: 40, backgroundColor: "var(--color-background)", borderBottom: "1px solid var(--color-border)", padding: "var(--space-4)", height: "72px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ width: "120px", height: "36px", backgroundColor: "var(--color-background-subtle)", borderRadius: "var(--radius-md)" }} />
        <div style={{ width: "40px", height: "40px", backgroundColor: "var(--color-background-subtle)", borderRadius: "var(--radius-md)" }} />
      </div>
    )
  }
  ...
  return (
    <>
      {/* Mobile Header (نظيف جداً: شعار + زر قائمة فقط) */}
      <div className="lg:hidden" style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={() => setIsOpen(!isOpen)} style={{
            width: "40px", height: "40px", borderRadius: "var(--radius-md)", backgroundColor: "var(--color-background-subtle)",
      ...
      <aside className="lg:hidden" style={{
        ...
        transition: "transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)", overflowY: "auto", boxShadow: "var(--shadow-xl)"
      }}>
      ...
      <div style={{ padding: "var(--space-4)", borderTop: "1px solid var(--color-border-light)", backgroundColor: "var(--color-background-subtle)" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
          ...
          <button onClick={() => signOut({ callbackUrl: "/" })} style={{
            display: "flex", alignItems: "center", gap: "var(--space-3)", padding: "var(--space-3) var(--space-4)",
            borderRadius: "var(--radius-lg)", fontSize: "var(--text-sm)", fontWeight: "600", color: "var(--color-danger)",
            backgroundColor: "transparent", border: "1px solid transparent", cursor: "pointer", width: "100%", textAlign: "right"
          }}>
