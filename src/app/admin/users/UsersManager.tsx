...
import { Shield, Key, UserCog, RefreshCw, CheckCircle2, XCircle, Eye, EyeOff, Music } from "lucide-react"
...
interface User {
  ...
  createdAt: string
  managedArtist: ManagedArtist | null
}

const ROLE_LABELS: Record<string, string> = {
  SUPER_ADMIN: "مدير عام",
  ADMIN: "مشرف",
  ARTIST_MANAGER: "مدير أعمال",
  USER: "مستخدم",
}
...
export default function UsersManager() {
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    try {
      ...
      setMessage({ type: "error", text: "فشل إعادة التعيين" })
    } finally {
      setResettingId(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">{users.length} مستخدم مسجل</p>
        <button onClick={fetchUsers} className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-[#1a1a1a] rounded-xl text-sm font-bold hover:bg-gray-200 dark:hover:bg-[#222] transition">
                ...
                <div>
                  ...
                  {user.managedArtist && (
                    <p className="text-xs text-purple-600 dark:text-purple-400 flex items-center gap-1 mt-0.5">
                      <Music size={10} /> مدير أعمال: {user.managedArtist.name}
                    </p>
                  )}
                </div>
              </div>
              ...
              {user.role !== "SUPER_ADMIN" && (
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-xs font-bold text-gray-500 dark:text-gray-400 flex items-center gap-1">
                    <Shield size={12} /> تغيير الدور:
                  </span>
                  {["USER", "ARTIST_MANAGER", "ADMIN"].map((role) => (
                    <button
                      key={role}
                      onClick={() => handleRoleChange(user.id, role)}
                      disabled={changingRoleFor === user.id || user.role === role}
                      className={`px-3 py-1 rounded-lg text-xs font-bold transition ${user.role === role ? "bg-[#D4AF37]/20 text-[#b8941f] cursor-default" : "bg-gray-100 dark:bg-[#1a1a1a] text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-[#222]"}`}
                    >
                      {ROLE_LABELS[role]}
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="relative flex-1 min-w-[200px] max-w-[300px]">
                  <input
                    ...
                    className="w-full px-3 py-2 pr-8 border border-gray-200 dark:border-gray-700 dark:bg-[#1a1a1a] dark:text-white rounded-lg text-sm focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent"
                  />
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  onClick={() => setNewPasswords((prev) => ({ ...prev, [user.id]: generatePassword() }))}
                  className="px-3 py-2 bg-gray-100 dark:bg-[#1a1a1a] rounded-lg text-xs font-bold hover:bg-gray-200 dark:hover:bg-[#222] transition"
                ...
                <button
                  ...
                  disabled={resettingId === user.id || !newPasswords[user.id]}
                  className="px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#b8941f] text-[#111] rounded-lg text-xs font-black hover:shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                >
                  {resettingId === user.id ? <RefreshCw size={12} className="animate-spin" /> : <Key size={12} />}
                  تعيين
                </button>
              </div>
            </div>
