...
export default async function BookingInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id) notFound();
  let b: any = null;
  try { b = await prisma.booking.findFirst({ where: { OR: [{ id }, { slug: id }] }, include: { artist: { select: { name: true, category: true } }, venue: { select: { name: true, address: true, city: true } } } }); } catch {}
  if (!b) notFound();
  const gross = Number(b.grossAmount || b.totalPrice || 0);
  const deposit = Number(b.depositAmount || b.deposit || 0);
  const remaining = Number(b.remainingAmount || b.remaining || gross - deposit);
  const dateStr = b.date ? new Date(b.date).toLocaleDateString("ar-EG", { year: "numeric", month: "long", day: "numeric" }) : "—";
  const sm: Record<string,string> = { PENDING:"معلق", CONFIRMED:"مؤكد", COMPLETED:"مكتمل", CANCELLED:"ملغي", REJECTED:"مرفوض" };
  return (
    <PrintLayout title="فاتورة حجز" docNumber={`INV-${(b.id||"").slice(0,8).toUpperCase()}`} verificationCode={b.id}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:24 }}>
        ...
        <div className="dash-card"><h3 style={{ color:"#D4AF37", marginBottom:10, fontSize:"11pt", fontWeight:800, borderBottom:"1px solid #D4AF37", paddingBottom:6 }}>بيانات الحجز</h3>
          <table className="print-info-table"><tr><td>الفنان</td><td>{b.artist?.name||"—"}</td></tr><tr><td>التصنيف</td><td>{b.artist?.category||"—"}</td></tr><tr><td>المكان</td><td>{b.venue?.name||b.location||"—"}</td></tr><tr><td>العنوان</td><td>{b.venue?.address||b.venue?.city||"—"}</td></tr></table></div>
      </div>
      ...
      <div className="dash-card" style={{marginTop:20}}><h3 style={{color:"#D4AF37",marginBottom:10,fontSize:"11pt",fontWeight:800,borderBottom:"1px solid #D4AF37",paddingBottom:6}}>تفاصيل الحجز</h3>
        <table className="print-info-table"><tr><td>تاريخ الفعالية</td><td>{dateStr}</td></tr><tr><td>وقت الحجز</td><td>{b.timeSlot||b.time||"—"}</td></tr><tr><td>الحالة</td><td><span className={`status-badge status-${(b.status||"").toLowerCase()}`}>{sm[b.status]||b.status||"—"}</span></td></tr>{b.notes&&<tr><td>ملاحظات</td><td>{b.notes}</td></tr>}</table></div>
    </PrintLayout>
  );
}
