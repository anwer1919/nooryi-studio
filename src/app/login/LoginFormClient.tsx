...
export default function LoginFormClient() {
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      ...
      setStep("success");
      setTimeout(() => { router.push(callbackUrl || "/"); router.refresh(); }, 1500);
  ...
  if (step === "otp") {
    ...
    return (
      <div className="w-full max-w-md py-8">
        <button onClick={() => { setStep("credentials"); setOtpDigits(["","","","","",""]); setError(""); setShowResendOptions(false); }} className="flex items-center gap-2 text-sm text-gray-500 dark:text-white/60 hover:text-[#b8941f] mb-6 transition-colors">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-white/60 text-sm">
            تم إرسال رمز التحقق إلى <span className="font-bold text-gray-800 dark:text-white/90">{otpInfo?.destination}</span>
          </p>
        </div>
        ...
        <div className="flex justify-center gap-3 mb-6" dir="ltr" onPaste={handleOtpPaste}>
          {otpDigits.map((digit, i) => (
            <input
              ...
              className="w-13 h-14 sm:w-14 sm:h-16 text-center text-2xl font-black text-gray-900 dark:text-white border-2 border-gray-200 dark:border-white/10 rounded-xl focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/30 outline-none transition-all bg-white dark:bg-[#0a0a0a]"
              style={{ width: "3.25rem" }}
            />
          ))}
        </div>
        ...
        <div className="mt-6 text-center">
          {!showResendOptions ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-gray-500 dark:text-white/60 mb-2">اختر طريقة الاستلام:</p>
              <button onClick={() => handleResend("email")} disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 border border-gray-200 dark:border-white/10 rounded-xl hover:bg-gray-50 dark:bg-white/5 transition text-sm font-semibold text-gray-700 dark:text-white/80 disabled:opacity-50">
                <Mail size={16} /> إرسال عبر البريد الإلكتروني
              </button>
              <button onClick={() => handleResend("whatsapp")} disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 border border-green-200 bg-green-50 rounded-xl hover:bg-green-100 transition text-sm font-semibold text-green-800 disabled:opacity-50">
                <MessageCircle size={16} /> إرسال عبر واتساب
              </button>
              <button onClick={() => handleResend("sms")} disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 border border-blue-200 bg-blue-50 rounded-xl hover:bg-blue-100 transition text-sm font-semibold text-blue-800 disabled:opacity-50">
                <Smartphone size={16} /> إرسال رسالة نصية SMS
              </button>
    ...
    <div className="w-full max-w-md py-8">
      ...
      <div className="space-y-3 mb-6">
        <button onClick={() => handleSocialLogin("google")} disabled={loading} className="w-full flex items-center justify-center gap-3 py-3.5 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-white/20 rounded-xl hover:bg-gray-50 dark:bg-white/5 hover:border-gray-400 hover:shadow-md transition-all cursor-pointer group disabled:opacity-50">
          ...
          <span className="text-sm font-bold text-gray-700 dark:text-white/80 group-hover:text-gray-900 dark:text-white">تسجيل الدخول بحساب Google</span>
        </button>
      ...
      <form onSubmit={handleCredentialsSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            ...
            <input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full pr-10 pl-4 py-3 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-[#d4af37] outline-none transition-all text-gray-900 dark:text-white placeholder:text-gray-400" placeholder="example@email.com" />
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} required value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} className="w-full pr-10 pl-10 py-3 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-[#d4af37] outline-none transition-all text-gray-900 dark:text-white placeholder:text-gray-400" placeholder="••••••••" />
