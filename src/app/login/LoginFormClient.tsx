...
export default function LoginFormClient() {
  ...
  // ═══ الخطوة 1: التحقق من البريد والباسورد ═══
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // أولاً: التحقق من صحة البيانات بدون تسجيل دخول
      ...
      // ثانياً: التحقق من الباسورد عبر signIn بدون redirect
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setOtpInfo({ method: data.method, destination: data.destination });
        setResendTimer(60);
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // الرمز صحيح! تسجيل الدخول النهائي
      setStep("success");
      setTimeout(() => {
  ...
  if (step === "otp") {
    return (
      <div className="w-full max-w-md py-8">
        <button onClick={() => { setStep("credentials"); setFormData({ ...formData, otp: "" }); setError(""); }} className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#b8941f] mb-6 transition-colors">
        ...
        <div className="mt-6 text-center">
          ...
          <button
            ...
            className="text-sm font-bold text-[#b8941f] hover:text-[#d4af37] transition-colors disabled:text-gray-400 disabled:cursor-not-allowed"
          >
