...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-bg overflow-x-hidden" dir="rtl">
        ...
        <div className="relative max-w-7xl mx-auto px-4 lg:px-8">
          ...
          <div className="flex items-center justify-center gap-5 mb-5 mt-4 md:mt-8">
            <div className="relative animate-float-up">
              ...
              <div className="relative w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-surface to-card border border-[#F5A623]/40 flex items-center justify-center shadow-2xl">
          ...
          <p className="text-center text-base md:text-lg lg:text-xl text-muted max-w-2xl mx-auto leading-relaxed mb-4 md:mb-6 px-4">
      ...
      <section id="services" className="py-16 md:py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="text-center mb-12 md:mb-16">
            ...
            <p className="text-muted max-w-2xl mx-auto text-sm md:text-base">نوفر لك أعلى معايير الجودة والاحترافية في كل خطوة</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <div key={i} className="group bg-bg rounded-2xl md:rounded-3xl p-6 md:p-8 border border-[#F5A623]/10 hover:border-[#F5A623]/30 transition-all duration-300 hover:-translate-y-1">
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-surface border border-[#F5A623]/20 flex items-center justify-center mb-5 md:mb-6 group-hover:scale-110 transition-transform">
                  <f.icon size={28} className="text-[#F5A623]" />
                </div>
                <h3 className="text-lg md:text-xl font-black text-fg mb-2 md:mb-3">{f.title}</h3>
                <p className="text-muted leading-relaxed text-sm md:text-base">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-center">
            <div>
              ...
              <p className="text-muted leading-relaxed mb-4 md:mb-6 text-sm md:text-base">نحن منصة رائدة في مجال حجز الفنانين والموسيقيين للفعاليات والمناسبات. نجمع بين أفضل المواهب الفنية في مكان واحد.</p>
              <p className="text-muted leading-relaxed mb-6 md:mb-8 text-sm md:text-base">سواء كنت تبحث عن فرقة موسيقية لحفل زفاف، أو مغني لفعالية خاصة، أو منسق أغاني — تجد لدينا كل ما تحتاجه.</p>
              <div className="space-y-3">
                {["أكثر من 150 فنان محترف معتمد", "نظام دفع آمن ومشفر 100%", "دعم فني على مدار الساعة", "ضمان استرداد كامل في حالة الإلغاء"].map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="text-muted font-semibold text-sm md:text-base">{item}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative">
              <div className="bg-surface rounded-2xl md:rounded-3xl p-6 md:p-8 border border-[#F5A623]/10">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div><p className="text-lg md:text-xl font-black text-fg">{siteSettings.siteName}</p><p className="text-sm text-muted">{siteSettings.tagline}</p></div>
                </div>
                <div className="space-y-3">
                  {[{ label: "سنوات الخبرة", value: "+5" }, { label: "الفنانين المعتمدين", value: "+150" }, { label: "الفعاليات الناجحة", value: "+500" }, { label: "رضا العملاء", value: "98%" }].map((stat, i) => (
                    <div key={i} className="flex items-center justify-between p-3 md:p-4 bg-bg rounded-xl border border-[#F5A623]/5">
                      <span className="text-muted font-semibold text-sm md:text-base">{stat.label}</span>
      ...
      {/* ═══ CTA ═══ */}
      <section className="py-16 md:py-20 bg-gradient-to-br from-surface to-card">
        <div className="max-w-4xl mx-auto px-4 lg:px-8 text-center">
          ...
          <p className="text-base md:text-lg text-muted mb-8 md:mb-10 max-w-2xl mx-auto">انضم إلى آلاف العملاء الذين يثقون بنا في تنظيم فعاليات لا تُنسى</p>
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
            ...
            <Link href="/login" className="inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-bg border-2 border-[#F5A623]/30 text-fg rounded-2xl font-black text-base md:text-lg hover:border-[#F5A623] transition-all">تسجيل الدخول</Link>
          </div>
        </div>
      </section>

      {/* ═══ تواصل معنا ═══ */}
      <section id="contact" className="py-16 md:py-20 bg-bg">
        <div className="max-w-6xl mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {[
              { icon: Phone, title: "اتصل بنا", value: siteSettings.phone, ltr: true, bg: "bg-surface" },
              { icon: Mail, title: "راسلنا", value: siteSettings.email, ltr: true, bg: "bg-gradient-to-br from-[#F5A623] to-[#E8961A]", iconDark: true },
              { icon: MapPin, title: "موقعنا", value: siteSettings.address, ltr: false, bg: "bg-surface" },
            ].map((c, i) => (
              <div key={i} className="bg-surface rounded-2xl p-6 md:p-8 text-center border border-[#F5A623]/10 hover:border-[#F5A623]/30 transition">
                ...
                <h3 className="text-base md:text-lg font-black text-fg mb-2">{c.title}</h3>
                <p className="text-muted text-sm md:text-base" dir={c.ltr ? "ltr" : "rtl"}>{c.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ═══ Footer ══ */}
      <footer className="bg-surface border-t border-[#F5A623]/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><p className="text-lg md:text-xl font-black text-fg">Nooryi</p><p className="text-[9px] text-[#F5A623] font-bold tracking-[0.2em] uppercase">{siteSettings.siteName.split(" ")[1] || "Studio"}</p></div>
              </div>
              <p className="text-muted text-sm leading-relaxed max-w-md">منصة احترافية لحجز أفضل الفنانين والموسيقيين للفعاليات والمناسبات.</p>
            </div>
            <div>
              <h4 className="text-sm font-black text-white mb-4">روابط سريعة</h4>
              <ul className="space-y-2 text-sm">
                <li><Link href="/" className="text-muted hover:text-[#F5A623] transition">الرئيسية</Link></li>
                <li><Link href="/artists" className="text-muted hover:text-[#F5A623] transition">الفنانين</Link></li>
                <li><Link href="/#about" className="text-muted hover:text-[#F5A623] transition">من نحن</Link></li>
                <li><Link href="/#services" className="text-muted hover:text-[#F5A623] transition">خدماتنا</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="text-sm font-black text-white mb-4">حسابك</h4>
              <ul className="space-y-2 text-sm">
                <li><Link href="/login" className="text-muted hover:text-[#F5A623] transition">تسجيل الدخول</Link></li>
                <li><Link href="/register" className="text-muted hover:text-[#F5A623] transition">إنشاء حساب</Link></li>
                <li><Link href="/my-bookings" className="text-muted hover:text-[#F5A623] transition">حجوزاتي</Link></li>
              </ul>
            </div>
          </div>
          <SocialLinks />
          <div className="pt-6 md:pt-8 border-t border-[#F5A623]/10 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-muted">© 2026 {siteSettings.siteName}. جميع الحقوق محفوظة.</p>
