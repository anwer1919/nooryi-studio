...
function BookingForm() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-muted text-xl font-bold mb-4">الفنان غير موجود</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#faf8f0] to-white dark:from-bg dark:to-[#111]" dir="rtl">
      <header className="bg-white/95 dark:bg-surface/95 backdrop-blur-xl border-b border-[#e8e4d9] dark:border-gray-800 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
          <Link href={`/artists/${slug}`} className="flex items-center gap-2 text-gray-700 dark:text-muted hover:text-[#E8961A]">
            <ArrowRight size={18} />
            <span className="font-bold">العودة</span>
          </Link>
          <h1 className="text-lg font-black text-gray-900 dark:text-fg">حجز {artist?.name || "الفنان"}</h1>
      ...
      <main className="max-w-5xl mx-auto px-4 lg:px-8 py-8">
          ...
          <div className="card-pro p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 dark:bg-card dark:text-fg rounded-2xl focus:ring-2 focus:ring-[#F5A623] focus:border-transparent"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pr-12 pl-4 py-3 border border-gray-200 dark:border-gray-700 dark:bg-card dark:text-fg rounded-2xl focus:ring-2 focus:ring-[#F5A623] focus:border-transparent"
            ...
            <div>
              ...
              <div className="relative">
                <Mail size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted" />
                <input
                  type="email"
        ...
        {step === 2 && (
          <div className="card-pro p-6 space-y-4">
            ...
            {regions.length > 0 && (
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 dark:bg-card dark:text-fg rounded-2xl focus:ring-2 focus:ring-[#F5A623]"
                >
            ...
            {/* ملخص السعر */}
            <div className="bg-gradient-to-br from-surface to-[#232323] p-5 rounded-2xl text-fg">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted">السعر الأساسي</span>
                <span className="font-bold">{(artist?.basePrice || artist?.minPrice || 1000).toLocaleString()} ج.م</span>
              </div>
              {selectedRegion && (
                <>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-muted">رسوم السفر ({selectedRegion.regionName})</span>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setStep(1)}
                className="px-6 py-4 bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-fg rounded-2xl font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition"
              >
                السابق
              </button>
              <button
        ...
        {step === 3 && (
          <div className="card-pro p-6 space-y-4">
            ...
            {/* ملخص نهائي */}
            <div className="bg-gray-50 dark:bg-card p-5 rounded-2xl space-y-2 text-sm">
              <h3 className="font-black text-gray-900 dark:text-fg mb-3">ملخص الحجز</h3>
              <div className="flex justify-between">
                <span className="text-muted dark:text-muted">الفنان</span>
                <span className="font-bold text-gray-900 dark:text-fg">{artist?.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted dark:text-muted">الاسم</span>
                <span className="font-bold text-gray-900 dark:text-fg">{form.clientName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted dark:text-muted">الهاتف</span>
                <span className="font-bold text-gray-900 dark:text-fg" dir="ltr">{form.clientPhone}</span>
              </div>
              {selectedRegion && (
                <div className="flex justify-between">
                  <span className="text-muted dark:text-muted">المنطقة</span>
                  <span className="font-bold text-gray-900 dark:text-fg">{selectedRegion.regionName}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-muted dark:text-muted">التاريخ</span>
                <span className="font-bold text-gray-900 dark:text-fg">
                  {form.date ? new Date(form.date).toLocaleDateString("ar-EG") : "—"}
                </span>
              </div>
              <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                <span className="font-black text-gray-900 dark:text-fg">المبلغ الإجمالي</span>
