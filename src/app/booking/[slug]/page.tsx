...
export default function BookingPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/artists/${slug}`).then((r) => r.json()),
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const res = await fetch(`/api/artists/${slug}/book`, {
        method: "POST",
  ...
  const canProceed = () => {
    ...
    if (step === 2) return form.clientName && form.clientPhone && form.venueId;
    return true;
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#faf8f0] to-white" dir="rtl">
      ...
      <header className="bg-white/95 backdrop-blur-xl border-b border-[#e8e4d9] sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
          <Link href={`/artists/${slug}`} className="flex items-center gap-2 text-gray-700 hover:text-[#b8941f] transition">
            <ArrowRight size={18} />
            ...
            <div key={s} className="flex-1 flex items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm ${
                  step >= s
                    ? "bg-gradient-to-br from-[#d4af37] to-[#b8941f] text-[#111]"
              ...
              {s < 3 && (
                <div className={`flex-1 h-1 mx-2 rounded ${step > s ? "bg-[#d4af37]" : "bg-gray-200"}`}></div>
              )}
            </div>
          ))}
        </div>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">معلومات للتواصل معك بشأن الحجز</p>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-4 bg-[#faf8f0] rounded-2xl">
                    ...
                    <span className="font-black text-gray-900">{form.timeSlot || "—"}</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-[#faf8f0] rounded-2xl">
                    <span className="font-bold text-gray-700">نوع الفعالية</span>
                    <span className="font-black text-gray-900">{form.eventType || "—"}</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-[#faf8f0] rounded-2xl">
                    <span className="font-bold text-gray-700">العميل</span>
                    <span className="font-black text-gray-900">{form.clientName}</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-[#faf8f0] rounded-2xl">
                    <span className="font-bold text-gray-700">الهاتف</span>
                    <span className="font-black text-gray-900" dir="ltr">{form.clientPhone}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="sticky top-24 card-pro p-6 space-y-5">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-black text-gray-900">
                    {(artist.basePrice || 1000).toLocaleString()} ج.م
                  </span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xl font-black gold-text">
                    {Math.round((artist.basePrice || 1000) * 1.1).toLocaleString()} ج.م
                  </span>
                </div>
              </div>
              ...
              <div className="pt-5 border-t border-[#e8e4d9] space-y-2 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  <CheckCircle2 size={14} className="text-[#b8941f]" />
